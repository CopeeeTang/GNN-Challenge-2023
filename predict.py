...
def _default_individual_prediction(model: tf.keras.Model, sample: any) -> np.ndarray:
    ...
    # Transform the prediction from ms to s, and return 最后单位为s
    return  pred / 1000
...
def predict(
    ...
    predict_file_path: Optional[str] = 'predict',
    ...
    if verbose:
        print()
    #初始化一系列列表来存储样本 ID、流 ID 和预测延迟。在数据集 ds 上迭代，
    #对每个样本进行处理，使用 individual_prediction 函数获取预测结果。
    ...
    #证每个样本的预测流数量是否与预期相符。如果有不符合的情况，输出错误信息并退出程序
...
if __name__ == "__main__":
    import argparse
    import models
    from train import get_mean_std_dict
