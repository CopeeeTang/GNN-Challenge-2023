...
```bash
python predict.py -ds (CBR+MB|MB) --ckpt-path "path/to/checkpoint"一定是具体checkpoints，不带index索引 --tr-path "path/to/training/dataset" --te-path "path/to/test/dataset" [--toy]
```
