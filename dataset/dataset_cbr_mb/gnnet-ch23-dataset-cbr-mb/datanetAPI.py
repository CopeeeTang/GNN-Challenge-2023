...
class Sample:
    ...
    max_link_load: double
        This variable is used in our testbed to indicate the maximum utilization 最大利用率
        of the links of the testbed.  
    performance_matrix : NxN matrix
        Matrix where each cell [i,j] contains aggregated 总共包含的transmission parameters
        information about transmission parameters between source i and
        destination j.
    traffic_matrix : NxN matrix size and time distributions
        Matrix where each cell [i,j] contains aggregated
        information about size and time distributions between source i and
        destination j.
    routing_matrix : NxN matrix path
